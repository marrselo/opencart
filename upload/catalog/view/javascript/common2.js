...
$(document).ready(function() {
	...
	{
	 if ($(window).width() <= 580 && responsiveflag == false)
	 {
	...
	function accordionFooter(status)
	{
	  ...
	  {
	   $('.footer .footer-title').removeClass('active').off().parent().find('.toggle-footer').removeAttr('style').slideDown('fast');
...
function display(view) {
	if (view == 'list') {
		...
		$('#content .product-layout').attr('class', 'product-layout product-list');
		$('.product-list > div').each(function(index, element) {
			html = '<div class="item_product"><div class="item-inner">';
			html += '<div class="left-block col-xs-12 col-sm-4 col-md-4"><div class="image">';
				html += ' <div class="box-label">' + $(element).find('.box-label').html() + '</div>';
				html += ' <div class="img-other">' + $(element).find('.img-other').html() + '</div>';
			html += '</div></div>';
			html += '<div class="right-block col-xs-12 col-sm-8 col-md-8"><div class="caption">';
				...
				html += $(element).find('.actions').html() ;
				html += '</div>';
			html += '</div></div></div>';
			$(element).html(html);
